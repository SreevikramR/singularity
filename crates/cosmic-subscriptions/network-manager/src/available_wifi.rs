...
pub async fn handle_wireless_device(
    ...
    if device.request_scan(HashMap::new()).await.is_ok() {
        let mut scan_changed = device.receive_last_scan_changed().await;
        if let Ok(Some(t)) = tokio::time::timeout(std::time::Duration::from_secs(4), scan_changed.next()).await {
            if let Ok(-1i64) = t.get().await {
                tracing::debug!("wireless device scan returned -1");
            }
        }
    }
