...
pub async fn change_adapter_status(
    ...
    let rfkill_bin = if std::path::Path::new("/usr/sbin/rfkill").exists() {
        "/usr/sbin/rfkill"
    } else if std::path::Path::new("/sbin/rfkill").exists() {
        "/sbin/rfkill"
    } else {
        "rfkill"
    };

    let mut cmd = tokio::process::Command::new(rfkill_bin);
