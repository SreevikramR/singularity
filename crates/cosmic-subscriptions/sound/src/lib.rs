...
pub fn watch() -> impl Stream<Item = Message> + MaybeSend + 'static {
    cosmic::iced_futures::stream::channel(1, |mut emitter: futures::channel::mpsc::Sender<Message>| async move {
        loop {
...
#[derive(Default)]
pub struct Model {
    subscription_handle: Option<Arc<SubscriptionHandle>>,
    ...
    node_volumes: IntMap<NodeId, u32>,
    node_mutes: IntMap<NodeId, bool>,
...
impl Model {
        ...
        match event {
            pipewire::Event::NodeProperties(id, props) => {
                if let Some(mute) = props.mute {
                    self.node_mutes.insert(id, mute);
                }
                
                if let Some(ref channel_volumes) = props.channel_volumes {
                    let (volume, _) = pipewire::volume::from_channel_volumes(channel_volumes);
                    self.node_volumes.insert(id, (volume * 100.0) as u32);
                }
    ...
    fn remove_node(&mut self, id: NodeId) {
        ...
        self.node_volumes.remove(id);
        self.node_mutes.remove(id);
    ...
    fn set_default_sink_id(&mut self, node_id: NodeId) {
        ...
        if let Some(&vol) = self.node_volumes.get(node_id) {
            self.sink_volume = vol;
            self.sink_volume_text = numtoa::BaseN::<10>::u32(vol).as_str().to_owned();
        }
        if let Some(&mute) = self.node_mutes.get(node_id) {
            self.sink_mute = mute;
        }
    }
    ...
    fn set_default_source_id(&mut self, node_id: NodeId) {
        ...
        if let Some(&vol) = self.node_volumes.get(node_id) {
            self.source_volume = vol;
            self.source_volume_text = numtoa::BaseN::<10>::u32(vol).as_str().to_owned();
        }
        if let Some(&mute) = self.node_mutes.get(node_id) {
            self.source_mute = mute;
        }
...
pub async fn set_default(id: u32) {
    ...
    let id_str = numtoa::BaseN::<10>::u32(id);
    
    let wpctl_result = tokio::process::Command::new("wpctl")
    ...
    if wpctl_result.is_err() || !wpctl_result.unwrap().success() {
        tracing::debug!(target: "sound", "wpctl failed, falling back to pw-cli for node {id}");
